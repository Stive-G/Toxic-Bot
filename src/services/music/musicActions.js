...
export async function playQuery(client, interaction, query) {
    ...
    if (
        ...
        if (!track) {
            throw new ToxicBotError('No results', ErrorTypes.USER_INPUT, 'No results found for that query.');
        }
    ...
    throw new ToxicBotError('No results', ErrorTypes.USER_INPUT, `No results found. (loadType: ${loadType})`);
}

export async function skipTrack(client, interaction) {
    const player = getPlayer(client, interaction.guild.id);
    if (!player?.current) {
        throw new ToxicBotError('No player', ErrorTypes.USER_INPUT, 'Nothing is playing right now.');
    }
...
export async function stopPlayback(client, interaction) {
    const player = getPlayer(client, interaction.guild.id);
    if (!player) {
        throw new ToxicBotError('No player', ErrorTypes.USER_INPUT, 'No active music player.');
    }
    assertCanControl(interaction.member, player);
...
export async function pausePlayback(client, interaction) {
    ...
    if (player.paused) {
        throw new ToxicBotError('Already paused', ErrorTypes.USER_INPUT, 'Playback is already paused.');
    }
...
export async function resumePlayback(client, interaction) {
    ...
    if (!player.paused) {
        throw new ToxicBotError('Not paused', ErrorTypes.USER_INPUT, 'Playback is not paused.');
    }
...
export async function shuffleQueue(client, interaction) {
    ...
    if (!player?.queue?.length) {
        throw new ToxicBotError('Empty queue', ErrorTypes.USER_INPUT, 'The queue is empty.');
    }
    assertCanControl(interaction.member, player);
...
export async function removeFromQueue(client, interaction, index) {
    ...
    if (queueIndex < 0 || queueIndex >= player.queue.length) {
        throw new ToxicBotError('Invalid index', ErrorTypes.USER_INPUT, `Invalid queue position. Queue has ${player.queue.length} track(s).`);
    }
...
export async function moveInQueue(client, interaction, from, to) {
    ...
    if (fromIndex < 0 || fromIndex >= player.queue.length || toIndex < 0 || toIndex >= player.queue.length) {
        throw new ToxicBotError('Invalid index', ErrorTypes.USER_INPUT, 'Invalid queue positions.');
    }
...
export async function clearQueue(client, interaction) {
    const player = getPlayer(client, interaction.guild.id);
    if (!player?.queue?.length) {
        throw new ToxicBotError('Empty queue', ErrorTypes.USER_INPUT, 'The queue is already empty.');
    }
    assertCanControl(interaction.member, player);
    player.queue.clear();
    await refreshPlayerMessage(client, interaction.guild.id);
...
export async function leaveVoiceChannel(client, interaction) {
    ...
    const player = getPlayer(client, guildId);
    if (!player) {
        throw new ToxicBotError('No player', ErrorTypes.USER_INPUT, 'I am not in a voice channel.');
    }
    assertCanControl(interaction.member, player);
