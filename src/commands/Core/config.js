...
export default {
  ...
  async execute(interaction) {
    ...
    if (subcommand === 'reset') {
      if (!interaction.options.getBoolean('confirm')) return InteractionHelper.safeReply(interaction, { content: 'Réinitialisation annulée : confirmez avec `confirm: true`.', ephemeral: true });
      await ConfigService.reset(interaction.client, interaction.guildId, null, interaction.user.id);
      return InteractionHelper.safeReply(interaction, { embeds: [createEmbed({ title: 'Configuration réinitialisée', description: 'Les réglages principaux ont été remis à zéro.', color: 'success', footer: 'Toxic Bot • Configuration' })], ephemeral: true });
    }
    ...
    return InteractionHelper.safeReply(interaction, { embeds: [createEmbed({ title: `Configuration — ${interaction.guild.name}`, color: 'primary', footer: 'Toxic Bot • Configuration', fields: [
